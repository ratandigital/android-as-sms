...
class Ussd extends Entity implements JsonSerializable
{
     ...
     * @throws Exception
     */
    public static function notifyUpdates(User $user) {
        if (Setting::get("pusher_enabled")) {
            try {
                $where = $user->getIsAdmin() ? "" : " WHERE Ussd.userID = {$user->getID()}";
                $query = "SELECT COUNT(IF(Ussd.responseDate IS NULL, 1, NULL)) as totalPending, COUNT(IF(Ussd.responseDate IS NOT NULL, 1, NULL)) as totalSent FROM Ussd" . $where . " GROUP BY deviceID";
                $counts = MysqliDb::getInstance()->rawQuery($query);
                sendPusherNotification("user-{$user->getApiKey()}", "ussd-request", $counts);
            } catch (Exception $e) {
