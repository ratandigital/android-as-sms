...
class DeviceUser extends Entity
{
    ...
    public function sendUssdRequest(string $request, ?int $simSlot = null): Ussd
    {
        ...
        $response = $this->getDevice()->sendPushNotification($data)->wait();
        if ($response->getStatusCode() === 200) {
            ...
            if ($result["success"] != '1') {
                ...
                throw new Exception(strval($this) . " : {$error}");
            }
            if (!is_null($this->getUser()->getCredits())) {
                MysqliDb::getInstance()->rawQuery("UPDATE User SET credits = credits - 1 WHERE ID = {$this->getUserID()}");
            }
            MysqliDb::getInstance()->commit();
            Ussd::notifyUpdates($this->getUser());
            return $ussd;
        } else {
            throw new Exception(strval($this) . " : {$response->getStatusCode()} {$response->getReasonPhrase()}");
        }
    }
}