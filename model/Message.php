                ...
                if (is_null($user->getCredits()) || $user->getCredits() >= $messagesCount) {
                    ...
                    for ($i = 0; $i < count($messagesChunks); $i++) {
                        ...
                        if (!array_key_exists($identifiers[0], $groups)) {
                            ...
                            if ($deviceUser->getDevice()->getUseOwnerSettings() && $user->getID() != $deviceUser->getDevice()->getUserID()) {
                                $groups[$identifiers[0]] = ["device" => $deviceUser->getDevice(), "sims" => [], "data" => [], "settings" => ["delay" => $deviceUser->getDevice()->getUser()->getDelay(), "reportDelivery" => $deviceUser->getDevice()->getUser()->getReportDelivery(), "useProgressiveQueue" => $deviceUser->getDevice()->getUser()->isUseProgressiveQueue(), "sleepTime" => $user->getSleepTime(), "prioritize" => $prioritize]];
                            } else {
                                $groups[$identifiers[0]] = ["device" => $deviceUser->getDevice(), "sims" => [], "data" => [], "settings" => ["delay" => $user->getDelay(), "reportDelivery" => $user->getReportDelivery(), "useProgressiveQueue" => $user->isUseProgressiveQueue(), "sleepTime" => $user->getSleepTime(), "prioritize" => $prioritize]];
                            }
                        }
                    ...
                    if (isset($schedule)) {
                        if (!is_null($user->getCredits())) {
                            $user->setCredits($user->getCredits() - count($messages));
                            $user->save();
                        }
                    }
                    MysqliDb::getInstance()->commit();
